document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderOverviewCharts(data) {
        ...
        if (!distCanvas || !tfidfCanvas) { console.error("Canvas de gráficos não encontrados!"); return; }
        ...
        sortedAnalysis.forEach(([clusterId, clusterData]) => {
            ...
            html += `<div class="cluster-card mt-3">
                        <div class="cluster-card-header">
                            ...
                            <div class="mt-2 small text-muted" style="font-style: italic;"><strong>Top Palavras:</strong> "${clusterData.top_palavras.map(p => p.palavra).join(', ')}"</div>
                        </div>
                    </div>`;
        });
