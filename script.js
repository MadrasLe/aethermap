document.addEventListener('DOMContentLoaded', () => {
    ...
    function highlightPlot(highlightedIndices) {
        ...
        fullPlotData.forEach((point, index) => {
            if (highlightedIndices.has(index)) { highlightedData.push(point); } else { normalData.push(point); }
        });
        const traces = [
        ...
        const layout = {
            title: `Destacando ${highlightedIndices.size} Documentos Relevantes`, margin: { l: 0, r: 0, b: 0, t: 40 },
            scene: { xaxis: { title: 'UMAP 1' }, yaxis: { title: 'UMAP 2' }, zaxis: { title: 'UMAP 3' } },
            template: 'plotly_dark', paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)'
        };
